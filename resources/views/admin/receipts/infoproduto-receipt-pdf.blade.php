...
<head>
    ...
    @php
        $refNum = 'INF' . $compra->created_at->format('Y') . str_pad($compra->id, 7, '0', STR_PAD_LEFT);
        $paidAt = $compra->created_at->format('d/m/Y') . ' às ' . $compra->created_at->format('H:i');
        $vendedor = optional($compra->infoproduto)->user;
    @endphp
...
<div class="slip-wrap">
    ...
    <div class="slip-header">
        ...
        <div class="receipt-type">COMPROVATIVO DE COMPRA</div>
    </div>

    <div class="status-band">
        <span class="status-dot"></span>PAGO — CONCLUÍDO
    </div>
    ...
    <div class="slip-body">
        ...
        <div class="amount-block">
            ...
            <div class="amount-value">{{ number_format($compra->valor_pago, 2, ',', '.') }} <span class="amount-currency">Kz</span></div>
        </div>

        <div class="slip-row" style="font-size:10px;">
            <span class="lbl" style="color:#94a3b8;">Valor do Infoproduto</span>
            <span class="val" style="color:#94a3b8;">{{ number_format($compra->valor_pago, 2, ',', '.') }} Kz</span>
        </div>
        <div class="slip-row" style="font-size:10px;">
            <span class="lbl" style="color:#94a3b8;">Taxa Plataforma</span>
            <span class="val" style="color:#94a3b8;">{{ number_format($compra->comissao_plataforma, 2, ',', '.') }} Kz</span>
        </div>
        <div class="slip-row" style="font-size:10px;">
            <span class="lbl" style="color:#94a3b8;">Valor Líquido ao Vendedor</span>
            <span class="val" style="color:#94a3b8;">{{ number_format($compra->valor_freelancer, 2, ',', '.') }} Kz</span>
        </div>
        ...
        <div class="slip-row">
            ...
            <span class="val">{{ optional($compra->comprador)->name ?? '—' }}</span>
        </div>
        <div class="slip-row">
            <span class="lbl">E-mail</span>
            <span class="val">{{ optional($compra->comprador)->email ?? '—' }}</span>
        </div>

        <hr class="divider-dash">

        <div class="slip-row">
            <span class="lbl">Infoproduto</span>
            <span class="val">{{ Str::limit(optional($compra->infoproduto)->titulo ?? 'Infoproduto #' . $compra->infoproduto_id, 40) }}</span>
        </div>
        <div class="slip-row">
            <span class="lbl">Vendedor</span>
            <span class="val">{{ optional($vendedor)->name ?? '—' }}</span>
        </div>
