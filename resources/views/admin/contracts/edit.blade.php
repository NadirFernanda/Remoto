    ...
    <form method="POST" action="{{ route('admin.comercial.update', $contract) }}" enctype="multipart/form-data" class="max-w-xl mx-auto bg-slate-900/70 border border-white/10 rounded-2xl shadow p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="partner_name" class="w-full rounded-lg border border-white/10 bg-slate-950/60 text-white focus:border-[#00baff] focus:ring-2 focus:ring-[#00baff]/20 px-4 py-2 text-base transition" value="{{ $contract->partner_name }}" required>
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-2">Tipo</label>
                <input type="text" name="type" class="w-full rounded-lg border border-white/10 bg-slate-950/60 text-white focus:border-[#00baff] focus:ring-2 focus:ring-[#00baff]/20 px-4 py-2 text-base transition" value="{{ $contract->type }}" required placeholder="Fornecedor, Cliente, Parceria...">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="date" name="start_date" class="w-full rounded-lg border border-white/10 bg-slate-950/60 text-white focus:border-[#00baff] focus:ring-2 focus:ring-[#00baff]/20 px-4 py-2 text-base transition" value="{{ $contract->start_date }}">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-2">Data de Fim</label>
                <input type="date" name="end_date" class="w-full rounded-lg border border-white/10 bg-slate-950/60 text-white focus:border-[#00baff] focus:ring-2 focus:ring-[#00baff]/20 px-4 py-2 text-base transition" value="{{ $contract->end_date }}">
            </div>
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="notes" class="w-full rounded-lg border border-white/10 bg-slate-950/60 text-white focus:border-[#00baff] focus:ring-2 focus:ring-[#00baff]/20 px-4 py-2 text-base transition" rows="3">{{ $contract->notes }}</textarea>
        </div>
