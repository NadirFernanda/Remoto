<div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6 inline-flex items-center gap-3">
        <span class="text-xs text-gray-500">Total Saques:</span>
        <span class="text-xl font-bold text-red-500">{{ money_aoa($total) }}</span>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($logs as $log)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 text-sm text-right text-red-500">{{ money_aoa($log->valor) }}</td>
                        <td class="py-3 px-4 text-sm text-gray-500">{{ $log->descricao ?? '—' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="py-10 text-center text-sm text-gray-400">Sem saques para o período seleccionado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
