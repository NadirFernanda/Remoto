    ...
    <div class="bg-white border-b border-slate-100 shadow-sm">
        <div class="max-w-7xl mx-auto px-6 py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg shadow-violet-200/60">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @foreach($tabs as $val => $tab)
                <button wire:click="$set('categoryFilter', '{{ $val }}')"
                    class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold border transition-all {{ $categoryFilter === $val ? 'bg-indigo-600 border-indigo-600 text-white shadow-md shadow-indigo-200/50' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600' }}">
                                ...
                                <td class="py-3.5 px-4">
                                    @if($log->before || $log->after)
                                        <button wire:click="toggleExpand({{ $log->id }})"
                                            class="w-7 h-7 flex items-center justify-center rounded-lg transition-colors {{ $isExpanded ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-500 hover:bg-indigo-100 hover:text-indigo-600' }}">
