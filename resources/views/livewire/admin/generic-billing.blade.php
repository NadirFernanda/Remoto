        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            @foreach(['week' => 'Semana', 'month' => 'Mês', 'year' => 'Ano'] as $val => $lbl)
                <button wire:click="$set('period', '{{ $val }}')"
                    class="px-3 py-1.5 rounded-[10px] text-xs border transition {{ $period === $val ? 'bg-[#0070ff] text-white border-[#0070ff]' : 'bg-white text-gray-600 border-gray-200 hover:border-[#0070ff] hover:text-[#0070ff]' }}">
                    {{ $lbl }}
                </button>
            @endforeach
        </div>
