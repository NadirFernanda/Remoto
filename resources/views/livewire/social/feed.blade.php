<div class="max-w-2xl mx-auto py-8 px-4">
                ...
                @if(in_array(auth()->user()->activeRole(), ['freelancer', 'creator']))
                    <a href="{{ route('social.myposts') }}"
                       class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl border transition
                           {{ $myPostsOnly ? 'bg-[#00baff] text-white border-[#00baff]' : 'border-gray-200 text-gray-600 hover:border-[#00baff] hover:text-[#00baff]' }}">
    ...
    @if($isEmpty)
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-6 mb-6 text-center">
            ...
            <p class="text-xs text-gray-400 mt-1 mb-4">Explore os criadores e comece a seguir os que mais gosta.</p>
            <a href="{{ route('freelancers.search') }}"
               class="inline-flex items-center gap-1.5 text-sm text-[#00baff] font-semibold hover:underline">
                Descobrir criadores de conteúdo
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
