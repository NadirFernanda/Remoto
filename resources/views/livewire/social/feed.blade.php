<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-[#00c8ff] to-[#0033cc] rounded-2xl p-6 text-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-white/75 mt-1">Descubra e partilhe conteúdo da comunidade</p>
        </div>
        @auth
            @if(in_array(auth()->user()->activeRole(), ['freelancer', 'creator']))
                <a href="{{ route('social.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-white/15 border border-white/30 hover:bg-white/25 text-white transition self-start">
    ...
    <div class="relative">
        ...
        <input type="search" wire:model.live.debounce.400ms="search"
               placeholder="Pesquisar publicações ou pessoas..."
               class="w-full bg-white border border-gray-100 shadow-sm rounded-2xl pl-11 pr-4 py-3 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0055ff]/30 focus:border-[#0055ff]">
    </div>
