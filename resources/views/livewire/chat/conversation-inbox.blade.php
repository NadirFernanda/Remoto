<div class="max-w-4xl mx-auto space-y-6 chat-inbox-page" x-data x-init="setInterval(() => $wire.$refresh(), 30000)">
    ...
    <div class="relative">
        ...
        <input type="text"
               ...
               class="w-full pl-10 pr-4 py-3 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0055ff]/30 bg-white shadow-sm">
    </div>
        ...
        @forelse($services as $service)
            @php
                ...
                $statusColor = match($service->status) {
                    ...
                    'em_moderacao'       => 'bg-blue-50 text-[#0055ff] border-blue-200',
               ...
               class="flex items-center gap-4 p-4 bg-white rounded-2xl border transition-all group {{ $unread > 0 ? 'border-[#0055ff]/30 shadow-sm shadow-blue-100 hover:shadow-md hover:shadow-blue-100' : 'border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5' }}">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 mb-0.5">
                        <p class="text-sm font-bold text-gray-900 truncate group-hover:text-[#0055ff] transition-colors">
                ...
                {{-- Seta --}}
                <svg class="w-4 h-4 text-gray-300 flex-shrink-0 group-hover:text-[#0055ff] transition-colors" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 18l6-6-6-6"/>
                </svg>
            </a>

        @empty
            <div class="platform-empty-state flex flex-col items-center py-20 text-gray-400 bg-white rounded-2xl border border-gray-100">
