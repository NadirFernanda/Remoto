<div class="proposal-page max-w-5xl mx-auto space-y-6">
    ...
    <div class="proposal-list space-y-4">
                            ...
                            <div class="flex items-center gap-3 text-sm">
                                @if($proposal->value)
                                    <div class="flex items-center gap-1.5">
                                        <svg class="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2.5"/><path stroke-linecap="round" d="M6 9v.01M18 15v.01"/></svg>
        ...
        @empty
            <div class="proposal-empty flex flex-col items-center py-20 px-5 text-gray-400 bg-white rounded-2xl border border-gray-100">
                ...
                <p class="proposal-empty-title text-base font-semibold">Nenhuma proposta {{ $tab === 'pending' ? 'pendente' : ($tab === 'accepted' ? 'aceite' : 'recusada') }}</p>
                @if($tab === 'pending')
                    <p class="proposal-empty-message text-sm mt-1">Quando um cliente enviar uma proposta, ela aparecerá aqui.</p>
                @endif
            </div>
        @endforelse
    </div>
