<div class="max-w-6xl mx-auto space-y-6 freelancer-store-page">
    ...
    <div class="loja-hero rounded-2xl p-6 text-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button wire:click="openCreate"
            class="inline-flex items-center gap-1.5 bg-white/15 hover:bg-white/25 border border-white/30 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition whitespace-nowrap self-start">
    ...
    {{-- Feedback --}}
    @if($feedback)
        <div class="mb-6 px-4 py-3 rounded-2xl text-sm font-medium border shadow-sm {{ $feedbackType === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200' }}">
            {{ $feedback }}
        </div>
    @endif
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-3 mb-3">
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2 mt-1.5">
                                {{-- Type badge --}}
                                <span class="platform-badge text-xs font-medium bg-[#0055ff]/10 text-[#0055ff]">
                                    {{ $produto->tipoLabel() }}
                                </span>
                                {{-- Status badge --}}
                                <span class="platform-badge text-xs font-medium @if($produto->status === 'ativo') bg-green-100 text-green-700 @elseif($produto->status === 'em_moderacao') bg-yellow-100 text-yellow-700 @elseif($produto->status === 'inativo') bg-red-100 text-red-700 @else bg-gray-100 text-gray-700 @endif">
                                    {{ $produto->statusLabel() }}
                                </span>
                                {{-- Sponsored badge --}}
                                @if($produto->isPatrocinado())
                                    @php $pat = $produto->patrocinioAtivo(); @endphp
                                    <span class="platform-badge text-xs font-medium bg-amber-100 text-amber-700 flex items-center gap-1">
        ...
        <div class="mb-5">
            ...
            <div class="grid grid-cols-2 gap-3">
                <button type="button" wire:click="$set('sponsor_payment_method', 'wallet')"
                    class="flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all {{ $sponsor_payment_method === 'wallet' ? 'border-amber-500 bg-amber-50 text-amber-700 shadow-sm' : 'border-gray-100 bg-gray-50 text-gray-400 hover:border-amber-300 hover:bg-amber-50' }}">
                ...
                <button type="button" wire:click="$set('sponsor_payment_method', 'express')"
                    class="flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all {{ $sponsor_payment_method === 'express' ? 'border-amber-500 bg-amber-50 text-amber-700 shadow-sm' : 'border-gray-100 bg-gray-50 text-gray-400 hover:border-amber-300 hover:bg-amber-50' }}">
