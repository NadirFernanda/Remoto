...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="rounded-2xl p-6 text-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button wire:click="openForm('imagem')" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/15 border border-white/30 hover:bg-white/25 text-white text-sm font-semibold transition">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div wire:key="portfolio-item-{{ $item->id }}" class="bg-white border rounded-xl overflow-hidden shadow-sm group relative">
                        ...
                        @endif
                        @if($item->media_path)
                            <img src="{{ $item->mediaUrl() }}" alt="{{ $item->title }}" loading="lazy" decoding="async" class="w-full h-40 object-cover"
                                 onerror="this.closest('.group').querySelector('.portfolio-image-fallback')?.classList.remove('hidden'); this.remove();">
                            <div class="portfolio-image-fallback hidden w-full h-40 bg-gray-100 flex items-center justify-center text-gray-300">
