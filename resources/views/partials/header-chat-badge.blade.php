...
if (auth()->check()) {
    $_u    = auth()->user();
    $_sids = \App\Models\Service::where('cliente_id', $_u->id)
               ->orWhere('freelancer_id', $_u->id)
               ->pluck('id');
    foreach ($_sids as $_sid) {
        $_chatUnread += \App\Models\ChatRead::unreadCount($_sid, $_u->id);
    }
}
@endphp
<div x-data="{ unread: {{ (int) $_chatUnread }} }"
    ...
    <a href="{{ route('chat.inbox') }}"
       ...
       onmouseover="this.style.background='rgba(0,186,255,.13)';this.style.borderColor='rgba(0,186,255,.3)'"
    ...
    <span x-show="unread > 0"
          ...
          style="position:absolute;top:-4px;right:-4px;min-width:16px;height:16px;background:#ef4444;border-radius:999px;font-size:.65rem;font-weight:700;color:#fff;display:flex;align-items:center;justify-content:center;padding:0 3px;border:2px solid var(--site-header-bg,#0f1a2e);line-height:1;">
    </span>
</div>
