...
/**
 * Rede de segurança para confirmar pagamentos AppyPay de compras na Loja
 * mesmo que o webhook falhe. Mesma lógica do PollAppyPayChargeJob, mas para
 * InfoprodutoCompraCheckout — reconciliação partilhada via
 * AppyPayReconciliationService (idempotente).
 */
class PollAppyPayInfoprodutoCompraCheckoutJob implements ShouldQueue
{
    ...
    public function handle(AppyPayGateway $gateway, AppyPayReconciliationService $reconciliation): void
    {
        ...
        if (!$charge['success']) {
            Log::warning('PollAppyPayInfoprodutoCompraCheckoutJob: falha ao consultar estado', [
